...
def test_exercise_instance_creation() -> None:
    ...
    assert retrieved.session.total_minutes == 60
    session_db.close()
...
def test_exercise_instance_parameters_optional() -> None:
    ...
    instance = ExerciseInstance(
        session=practice_session, exercise=exercise, sequence_order=1, parameters={}
    )
...
def test_session_has_multiple_exercise_instances() -> None:
    ...
    instance1 = ExerciseInstance(
        session=practice_session, exercise=exercise1, sequence_order=1, parameters={}
    )
    ...
    retrieved_session = session_db.query(Session).first()
...
def test_exercise_instance_repr() -> None:
    ...
    instance = ExerciseInstance(
        session=practice_session, exercise=exercise, sequence_order=5, parameters={}
    )
    session_db.add(instance)
    session_db.commit()
